...
from loan_status_prediction.config import DATA_PATH, MODELS_DIR, RANDOM_STATE, REPORTS_DIR
from loan_status_prediction.data import load_loan_data, make_train_test_split
from loan_status_prediction.evaluation import BusinessCosts, evaluate_model
...
def run_comparison(
    ...
    for name, pipeline in candidate_pipelines(scale_pos_weight=scale_pos_weight).items():
        model, tuning_summary = tune_model(name, pipeline, X_train, y_train, n_iter, cv_splits)
        fitted_models[name] = model
        summaries.append(evaluate_model(name, model, X_test, y_test, costs))
    ...
    comparison = pd.DataFrame(summaries).sort_values("best_cost_business_cost")
    ...
    if save_best_model:
        ...
        artifact_metadata = {
            ...
            "selection_metric": "best_cost_business_cost",
            "threshold": best_row["best_cost_threshold"],
            "business_cost": best_row["best_cost_business_cost"],
